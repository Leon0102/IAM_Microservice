...
import { Controller, UseGuards, Get, Post, Body, Patch, Param, Req, Inject, Logger, Delete, NotFoundException } from '@nestjs/common';
...
export class UserController {
    ...
    // @MessagePattern({ role: 'user', cmd: 'getByEmail' })
    // getUserByEmail(data: any): Promise<User> {
    //   return this.userService.findOneByEmail(data.email);
    // }
    ...
    async createUser(@Body() user: CreateUserDto): Promise<void> {
        ...
        // this.client.emit('create', { username: user.username, password: user.password, email: user.email });
        // this.amqpConnection.publish('test', 'UserCreatedIntergrationEvent', result, { persistent: true });
        // this.amqpConnection.publish('logs', 'logs', { msg: "123123" }, { persistent: true });
    }
    ...
    async resetPassword(@Param('id') id: string): Promise<void> {
        return this.commandBus.execute(new ResetPasswordCommand(id));
    }
                                        ...
                                        <table border="0" cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td align="center" bgcolor="#1a82e2" style="border-radius: 6px;">
                                                    <a target="_blank" rel="noopener noreferrer" href="http://localhost:3010/users/reset-password/${user.uuid}" target="_blank"
