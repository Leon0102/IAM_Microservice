...
export class UserService {
    ...
    async findOneByToken(Token: string): Promise<User> {
        const result = await this.client.send({ role: 'auth', cmd: 'check' }, { jwt: Token }).toPromise();
        const user = await this.userRepository.findOneByEmail(result.email);
        if (!user) {
            throw new NotFoundException('User not found');
        }
        return user;
    ...
    async sendMail(email: string, userId: string) {
        ...
        // const url = `http://192.168.2.98:8000/api/users/reset-password?token=${token}`;
        const url = `http://localhost:8000/users/reset-password?token=${token}`;
                                        ...
                                        <table border="0" cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td align="center" bgcolor="#1a82e2" style="border-radius: 6px;">
                                                    <a target="_blank" rel="noopener noreferrer" href="${url}" target="_blank"
