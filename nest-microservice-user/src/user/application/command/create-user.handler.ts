...
export class CreateUserHandler implements ICommandHandler<CreateUserCommand> {
  ...
  async execute(command: CreateUserCommand): Promise<User> {
    try {
      ...
      const newUser = new User(command.name, command.password, command.email);
      ...
      this.amqpConnection.publish('test', 'UserCreatedIntergrationEvent', message, {
        type: 'direct',
        persistent: true,
      });
      ...
      const url = `http://localhost:8000/users/confirm-email?token=${token}`;
